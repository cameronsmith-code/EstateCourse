...
type Props = {
  index: number;
  data: Partial<CompanyOwedData>;
  client1Name: string;
  client2Name: string;
  hasSpouse: boolean;
  corporations: Array<{ legalName: string }>;
  onChange: (field: keyof CompanyOwedData, value: unknown) => void;
  onMultiChange: (updates: Partial<CompanyOwedData>) => void;
  onRemove: () => void;
};
...
export default function CompanyOwedDetails({
  index,
  data,
  client1Name,
  client2Name,
  ...
  const owedByLabel = data.owedBy === 'client1'
    ? client1Name
    : data.owedBy === 'client2'
      ? client2Name
      : data.owedBy === 'both'
  ...
  return (
    <div className="border border-gray-600 rounded-xl p-6 bg-gray-800 space-y-5 mt-2">
      ...
      <div>
        <label className={labelClass}>Who owes the company money?</label>
        <div className="space-y-2">
          {[
            ...
            <label key={opt.value} className="flex items-center p-3 border border-gray-600 bg-gray-700 rounded-lg hover:bg-gray-600 cursor-pointer">
              <input
                type="radio"
                name={`co-owedBy-${index}`}
                value={opt.value}
                checked={data.owedBy === opt.value}
                onChange={() => onMultiChange({ owedBy: opt.value })}
      ...
      {/* Which company is owed the money? */}
      <div>
        <label className={labelClass}>Which company is owed the money?</label>
        <select
      ...
      {/* Approximately how much does [Client] currently owe [Company]? */}
      <div>
        <label className={labelClass}>Approximately how much does {owedByLabel} currently owe {companyName}?</label>
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-1 italic">It's common for these balances to change throughout the year. An approximate current amount is fine.</p>
      </div>
        ...
        <div className="space-y-2">
          {[
            ...
            <label key={opt.value} className="flex items-center p-3 border border-gray-600 bg-gray-700 rounded-lg hover:bg-gray-600 cursor-pointer">
              <input
                type="radio"
                name={`co-hasWrittenAgreement-${index}`}
                value={opt.value}
      ...
      {/* Does the company charge interest? */}
      <div>
        <label className={labelClass}>Does the company charge interest on this amount?</label>
        <div className="space-y-2">
          {[
            { value: 'yes', label: 'Yes' },
            { value: 'no', label: 'No' },
        ...
        <div className="space-y-2">
          {[
            ...
            <label key={opt.value} className="flex items-center p-3 border border-gray-600 bg-gray-700 rounded-lg hover:bg-gray-600 cursor-pointer">
              <input
                type="radio"
                name={`co-hasRepaymentTerms-${index}`}
                value={opt.value}
