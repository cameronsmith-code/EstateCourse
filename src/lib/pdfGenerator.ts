...
export const generatePDF = (formData: FormData) => {
  ...
  const estateTrusteeIntro = 'Settling an estate in Canada is an intricate process that can involve over 60 individual tasks. These responsibilities represent a significant fiduciary burden that often requires months or even years of effort.';
  const splitIntro = doc.splitTextToSize(estateTrusteeIntro, fieldWidth);
  ...
  const addStageHeading = (stageText: string) => {
    if (yPosition > 260) {
      addPageFooter();
      doc.addPage();
      yPosition = 12;
    ...
    doc.setFont(undefined, 'bold');
    doc.setTextColor(...colors.navyBlue);
    doc.text(stageText, margin, yPosition);
    yPosition += 10;
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.setTextColor(...colors.darkText);
  ...
  const addTaskContent = (taskNumber: string, taskTitle: string, taskContent: string) => {
    if (yPosition > 270) {
      addPageFooter();
      doc.addPage();
      yPosition = 12;
    ...
    doc.setFont(undefined, 'bold');
    const taskHeader = `${taskNumber}. ${taskTitle}`;
    const splitHeader = doc.splitTextToSize(taskHeader, fieldWidth);
    ...
    doc.setFont(undefined, 'normal');
    const splitContent = doc.splitTextToSize(taskContent, fieldWidth);
  ...
  addStageHeading('Stage 1: Preliminary Steps');

  addTaskContent('1', 'Locate the original Will and any codicils',
    'The Will\'s location should be outlined in this document.');

  addTaskContent('2', 'Review the Will for specific funeral instructions',
    'The estate trustee should carefully review the Will to identify any specific instructions the deceased left regarding their funeral, memorial service, cremation, or burial preferences.');

  addTaskContent('3', 'Arrange the funeral, memorial, cremation, or burial',
    'Details on any funeral arrangements made by the deceased should be captured in this document previously.');

  addTaskContent('4', 'Ensure payment of funeral expenses is made or arranged',
    'The estate is ultimately responsible for the cost of the funeral, as these are considered debts or expenses of the succession that must be paid before assets are distributed to beneficiaries. While the Estate Trustee (or liquidator in Quebec) generally has the legal authority and responsibility to plan the funeral and arrange for payment, the funds themselves are drawn from the deceased\'s assets.\n\nPractical and legal considerations regarding funeral payments include:\n• Accessing Frozen Funds: Although a deceased person\'s bank accounts are typically frozen upon death, financial institutions generally allow the Estate Trustee to withdraw funds or write cheques directly from those accounts to pay for allowable expenses, which explicitly include funeral and cremation costs.\n• Trustee Indemnification: If an Estate Trustee chooses to pay for funeral arrangements out of their own pocket, they are entitled to be indemnified (reimbursed) from the estate property for these properly incurred expenses.\n• Quebec Variations: In Quebec, unless the deceased provided specific instructions in their Will, the heirs are responsible for making funeral arrangements rather than the liquidator, though the costs are still charged to the succession.');

  addTaskContent('5', 'Obtain multiple original copies of the proof-of-death certificate from the funeral director or province',
    'The estate trustee should obtain several certified copies of the death certificate, as multiple institutions will require original documentation.');

  addTaskContent('6', 'Assess whether the Will must be probated to confirm your authority',
    'The estate trustee reviews the deceased\'s assets, liabilities, and account structures to determine whether probate is necessary to formally confirm their authority to act.\n\nThis assessment considers factors such as asset values, ownership (sole vs. joint), beneficiary designations, and the requirements of financial institutions or land registries.\n\nWhere probate is required, the trustee prepares to apply for a Certificate of Appointment of Estate Trustee; where it is not, the trustee ensures sufficient documentation exists to administer the estate without court involvement while still meeting legal and institutional requirements.');

  addTaskContent('7', 'Apply to the appropriate provincial court for a Grant of Probate or Certificate of Appointment',
    'If probate is required, the estate trustee must prepare and submit the necessary application documents to the appropriate provincial court to obtain formal confirmation of their authority to act on behalf of the estate.');

  addTaskContent('8', 'Identify and review any marriage contracts, civil union agreements, or domestic contracts',
    'The location of marriage contracts, civil union agreements, or domestic contracts should be included in this document.');

  addTaskContent('9', 'Evaluate potential family law issues, such as claims on family patrimony or matrimonial regimes',
    'Evaluating family law issues is a mandatory preliminary step for an Estate Trustee to identify legal obligations that may curtail testamentary freedom and take priority over the distribution of assets. This involves reviewing matrimonial regime legislation and family patrimony (particularly in Quebec), which may require a portion of the estate\'s value to be paid to a surviving spouse or descendant regardless of the Will\'s terms. The representative must assess the deceased\'s marital status and province of residence, as provincial laws dictate whether rights to property division extend to common-law partners or remain unique to married couples. In certain jurisdictions, such as Ontario, a surviving spouse may elect to receive an equalization of family property instead of the bequests provided in the Will. To complete this evaluation, the trustee must locate and review any existing domestic contracts, such as marriage or cohabitation agreements, which may alter statutory rights to asset sharing.');

  addTaskContent('10', 'Assess potential dependant relief issues if the Will does not provide adequate support',
    'Assessing dependant relief issues involves identifying individuals who were financially dependent on the deceased and can demonstrate that the Will (or intestacy rules) failed to provide adequate support for their needs. All Canadian provinces have legislation allowing eligible dependants—typically spouses, minor children, and disabled adult children—to apply to the courts for an increased share of the estate\'s value.\n\nKey factors in this assessment include:\n• Legal Priority: Dependant relief claims can curtail testamentary freedom, and a court order may vary the terms of the Will or specific trusts to ensure support obligations are met.\n• Court Discretion: Judges have broad authority to determine what is "just and equitable" based on the applicant\'s degree of need, their ability to support themselves, and the length of time they were with the deceased.\n• Moral and Legal Obligations: In some jurisdictions, courts balance the testator\'s intentions against both the legal and moral obligations owed to family members, including independent adult children in certain provinces.\n• Asset Accessibility: Depending on the province, assets passing outside the estate, such as life insurance or registered plans, may sometimes be pulled back to satisfy a successful dependant relief claim.');

  addTaskContent('11', 'Calculate and pay probate taxes (Estate Administration Tax) to the provincial government',
    'Probate taxes (often called Estate Administration Tax) are fees imposed by provincial governments to validate a Will and officially confirm the executor\'s authority to manage the estate. These taxes are calculated based on the total fair market value of the assets that pass through the Will, though some provinces allow deductions for specific debts. Assets that pass directly to survivors or beneficiaries outside the Will—such as jointly owned property with right of survivorship or registered plans (RRSPs, RRIFs, TFSAs) and life insurance with named beneficiaries—are generally excluded from this calculation.\n\nProvincial rates vary significantly, ranging from nominal flat fees in Alberta and Quebec to higher percentage-based taxes in jurisdictions like Ontario, British Columbia, and Nova Scotia. For instance, Ontario charges $15 for every $1,000 of estate value above $50,000, while Nova Scotia levies a rate of approximately 1.695% on estates exceeding $100,000. The executor is responsible for paying these taxes to the provincial government at the time the probate application is submitted to the court.');

  addTaskContent('12', 'Ensure the family\'s immediate financial needs (cash flow) can be met during the initial transition',
    'Ensuring the family\'s immediate financial needs are met is a critical preliminary duty of an executor to ensure that dependents can continue to pay bills and manage daily expenses during the estate transition.\n\nKey strategies to secure this cash flow include:\n• Accessing Immediate Funding Sources: Once a proof-of-death certificate is obtained, the executor can apply for immediate funds from life insurance policies, employer death benefits, and company pension plans.\n• Utilizing Frozen Bank Accounts: While a deceased individual\'s accounts are typically frozen, many financial institutions will allow the executor to pay for "allowable expenses" directly from these funds, including funeral costs, property taxes, income taxes, utility bills, and home/auto insurance premiums.\n• Applying for Government Survivor Benefits: The executor should immediately apply for CPP/QPP survivor\'s pensions and children\'s benefits, as these provide recurring monthly support for eligible family members.\n• Leveraging Joint Ownership: Assets held in Joint Tenancy with Right of Survivorship (JTWROS), such as bank accounts or homes, generally pass automatically to the survivor by operation of law, providing much faster access to capital than assets that must go through the probate process.\n• Distributing for Immediate Needs: If the estate is clearly solvent, the executor may distribute specific assets or cash to beneficiaries to meet urgent requirements, provided they first ensure sufficient funds remain to cover all estate debts and taxes to avoid personal liability.');

  addStageHeading('Stage 2: Beneficiary Relations');

  addTaskContent('13', 'Meet with beneficiaries to set expectations regarding the timeline and process',
    'The executor\'s role is to clearly explain the estate process, outline expected timelines, and manage expectations around delays, decisions, and distributions—helping reduce uncertainty, conflict, and misunderstandings from the outset.');

  addTaskContent('14', 'Gather necessary personal information and Social Insurance Numbers (SIN) from beneficiaries',
    'This is fairly straightforward and can be done in conjunction with Step 13. Ensure safe and secure storage of confidential information, such as SIN numbers.');

  addTaskContent('15', 'Provide regular written status updates to prevent misunderstandings or conflict',
    'The executor should keep beneficiaries informed through clear, written, periodic updates on progress, timelines, and outstanding issues, helping maintain transparency and reduce the risk of confusion or conflict.');

  addTaskContent('16', 'Deliver a copy of the estate inventory (summary document) to all entitled beneficiaries',
    'The executor prepares and shares a clear summary of estate assets and liabilities with all entitled beneficiaries to ensure transparency and allow them to understand what forms the estate and how it is being administered.\n\nThe Will Companion Kit created in this document should assist in itemizing this information ahead of time. As circumstances change and evolve, this document will become stale, so updating it every few years is recommended.');

  addTaskContent('17', 'Communicate with residual beneficiaries specifically regarding the final distribution process',
    'The executor keeps residual beneficiaries informed as the estate nears completion, explaining how the final distribution will be calculated, the expected timing, and any remaining steps that must occur first (such as tax clearance). The executor should be preparing a final accounting of the estate, proposed distribution calculations, release forms, and supporting documentation so beneficiaries can review and understand the basis of the final payout before funds are distributed.');

  addTaskContent('18', 'Keep a detailed log of time spent on administration to justify executor compensation',
    'The estate trustee is responsible for maintaining an accurate record of all time and effort spent administering the estate to support any claim for executor compensation and to demonstrate that the work performed was reasonable and necessary.\n\nHow to best prepare the log:\nThe trustee should record entries contemporaneously, noting the date, task performed, time spent, and purpose (for example: asset management, beneficiary communication, professional coordination, or tax matters). The log should be factual, consistent, and supported where possible by emails, invoices, bank records, or meeting notes. Using a simple spreadsheet or dedicated log from the outset—rather than recreating time later—helps ensure credibility and reduces the risk of disputes if compensation is reviewed by beneficiaries or the court.');

  addStageHeading('Stage 3: Safeguarding and Identifying Assets');

  addTaskContent('19', 'Locate all physical assets, including vehicles, jewelry, and art',
    'The estate trustee identifies, secures, and documents all tangible assets of the deceased, such as vehicles, jewelry, art, and household contents. This involves inspecting residences and storage locations, safeguarding valuables, arranging appraisals where required, and recording each item for inclusion in the estate inventory and eventual distribution or sale.\n\nThe Will Companion Kit should outline these key, valuable physical assets and their location.');

  addTaskContent('20', 'Identify and secure digital assets, including social media accounts and cryptocurrencies',
    'The estate trustee identifies all digital assets, including online accounts, subscriptions, social media profiles, and cryptocurrencies, and takes steps to secure them by locating access information, preserving data, and preventing unauthorized use. This may involve working with service providers, digital wallets, or professionals to document the assets\' existence and value, manage or close accounts, and ensure they are properly included in the estate inventory and administered in accordance with the Will and applicable law.\n\nThe Will Companion Kit should outline these key digital assets for the Estate Trustee.');

  addTaskContent('21', 'Locate and retrieve all passwords or PINs associated with digital hardware and accounts',
    'The estate trustee gathers all available passwords, PINs, and access credentials for digital devices and online accounts to ensure the estate\'s digital assets can be accessed, secured, and properly administered. This may include reviewing records, password managers, written notes, and working with service providers or technical professionals, while carefully documenting access and maintaining security throughout the process.');

  addTaskContent('22', 'Formally notify all banks and financial institutions of the death',
    'The estate trustee contacts each bank and financial institution where the deceased held accounts to advise them of the death and request that accounts be restricted to prevent unauthorized transactions.\n\nThis typically involves providing a death certificate and, once available, proof of authority such as the Proof of Death, the Will and/or Certificate of Appointment of Estate Trustee. The trustee confirms account balances as of the date of death, arranges for statements to be issued to the estate, and ensures interest, dividends, and fees continue to be properly tracked for estate accounting purposes.\n\nA list of bank accounts should be found in the Will Companion Kit.');

  addTaskContent('23', 'Open an estate bank account to centralize all incoming funds and outgoing payments',
    'This will be done alongside Step 22. The estate trustee will open a dedicated estate bank account to receive all incoming funds—such as investment proceeds, refunds, and income—and to pay estate expenses, taxes, and distributions.\n\nCentralizing all transactions through a single account creates a clear audit trail, simplifies record-keeping, and helps demonstrate that estate funds are kept separate from the trustee\'s personal finances, which is essential for transparency, reporting, and final accounting to beneficiaries.');

  addTaskContent('24', 'Transfer all existing account balances to the new estate account',
    'Once authorized, the estate trustee arranges for balances from the deceased\'s personal bank and investment accounts to be transferred into the estate account. This consolidates estate funds, simplifies cash management, and ensures all income, expenses, and distributions flow through a single account. The trustee must confirm date-of-death values, retain statements showing the transfers, and ensure any registered or beneficiary-designated accounts are handled separately in accordance with the Will and applicable law.');

  addTaskContent('25', 'Cancel all credit card accounts held by the deceased',
    'The estate trustee notifies each credit card issuer of the death and requests that accounts be closed to prevent further charges or fraud. This involves providing a death certificate and, if required, proof of authority. The trustee confirms outstanding balances as of the date of death, ensures legitimate estate expenses are properly handled, and retains final statements for inclusion in the estate\'s liabilities and accounting.\n\nA list of credit cards should be found in the Will Companion Kit.');

  addTaskContent('26', 'Documentation and transfer of loyalty points (e.g., Aeroplan, Air Miles, credit card rewards)',
    'The estate trustee identifies all loyalty and rewards programs held by the deceased (such as airline miles, retail rewards, or credit card points) and reviews each program\'s terms to determine whether points can be transferred, redeemed, or must be forfeited on death.\n\nThe trustee documents point balances as of the date of death, notifies the program administrators, and provides required documentation (typically a death certificate and proof of authority). Where permitted, points may be transferred to a beneficiary, redeemed for estate expenses, or used in accordance with the Will.');

  addTaskContent('27', 'Verify and maintain adequate insurance coverage for all estate assets (home, auto, etc.)',
    'The estate trustee reviews all insurance policies covering estate assets—such as homes, vehicles, and valuable contents—to ensure coverage remains in force after death and reflects the change in ownership and occupancy. Insurers must be formally notified of the death, and policies may need to be amended, extended, or replaced to avoid gaps in coverage.\n\nMaintaining adequate insurance protects the estate from loss or liability during administration and helps the trustee fulfill their duty to preserve estate assets until they are sold or distributed.\n\nA list of property insurance policies should be found in the Will Companion Kit.');

  addTaskContent('28', 'Secure real property by changing locks and ensuring maintenance',
    'The estate trustee takes immediate steps to protect any real property by changing locks, securing entry points, and ensuring the home is safe from unauthorized access, vandalism, or damage.\n\nOngoing responsibilities include arranging basic maintenance such as heating, utilities, snow removal, lawn care, and regular inspections to preserve the property\'s condition and insurance coverage. Proper security and upkeep help protect the estate\'s value and reduce liability while the property is held, sold, or transferred.');

  addTaskContent('29', 'Arrange for property management if a home is vacant or a business must continue running',
    'This task, which typically falls under the safeguarding stage of settling an estate, requires the Estate Trustee (or liquidator) to take immediate steps to preserve the value of tangible and intangible assets until they can be sold or distributed.\n\nManaging Vacant Homes: When a property is left unoccupied following a death or during a period of incapacity, the legal representative must secure the premises to prevent loss, theft, or destruction.\n• Insurance Compliance: A critical risk is that standard homeowners\' insurance policies may be rendered invalid if a residence is left vacant for a specific period. The representative must notify the insurer of the vacancy and request to have their name added as the executor to the policy to maintain coverage.\n• Maintenance and Upkeep: Managing a home involves ongoing costs such as property taxes, utility bills, yard maintenance, and repairs. If the representative cannot handle these tasks personally, they should hire a property manager to ensure the asset does not deteriorate, which would otherwise decrease its fair market value.\n• Accessing Funds: Financial institutions generally allow an Estate Trustee to access the deceased\'s accounts specifically to pay for allowable expenses, which include property taxes and maintenance costs.');

  addTaskContent('30', 'Determine beneficial ownership of jointly held accounts to see if they belong to the estate',
    'In the context of settling an estate, determining the beneficial ownership of a jointly held account is a critical fiduciary task because the legal right of a survivor to withdraw funds does not automatically grant them the right to keep that money for their own use. While legal ownership refers to the individual whose name is registered on the account title, beneficial ownership describes the person or entity entitled to the actual benefits and enjoyment of the property. When a joint account is created gratuitously (where one person contributes the funds and adds another), the survivor\'s right to the proceeds depends on the intent of the original contributor at the time the account was established.');

  addStageHeading('Stage 4: Valuing the Estate');

  addTaskContent('31', 'Search the deceased\'s personal files and safety deposit boxes for original documents',
    'This document is designed to help the Estate Trustee(s) in locating the personal files of the deceased. The trustee should systematically search all locations where important documents may be stored.');

  addTaskContent('32', 'Locate original stock certificates, bonds, and property deeds',
    'This document is designed to help the Estate Trustee(s) in locating the personal files of the deceased. The trustee should identify and secure all original investment and property ownership documents.');

  addTaskContent('33', 'Identify and record the value of all assets precisely as of the date of death',
    'Identifying and recording the value of all assets as of the date of death is one of the most significant and demanding tasks an Estate Trustee (or liquidator) faces. This process is essential not only for fulfilling the instructions in the Will but also for determining provincial probate taxes and calculating the deemed disposition of capital property for the deceased\'s final tax return.\n\nKey requirements include:\n\n1. Determining Fair Market Value (FMV): For tax and legal purposes, the "value" of an asset is generally considered its fair market value. This is defined as the highest price obtainable in an open and unrestricted market between knowledgeable, informed, and prudent parties acting at arm\'s length. Under the Income Tax Act, a deceased person is deemed to have disposed of most property at its FMV at the moment immediately before death.\n\n2. Creating the Inventory of Assets: The trustee must prepare a detailed list that includes everything the testator owned or had an interest in, including financial records, physical and digital assets, and safe deposit box contents.\n\n3. Categorizing Assets (Estate vs. Non-Estate): Not all assets are included in the value of the estate for probate purposes. Assets with named beneficiaries (such as life insurance, RRSPs, or RRIFs) and property held in joint tenancy with right of survivorship generally pass directly to the survivor and are not subject to probate fees.\n\n4. Specialized Valuation Rules: Private corporation shares, foreign assets, and liabilities must all be carefully documented and valued. The personal representative should maintain a detailed log of time spent and a record of every financial transaction.');

  addTaskContent('34', 'Obtain professional appraisals for real estate, business interests, and high-value collectibles',
    'Independent Appraisals: For assets of substantial value or those where disagreement among beneficiaries is likely (such as real estate, businesses, or high-value collectibles), the trustee should obtain independent professional appraisals. The fees for these appraisals are considered a selling cost and can reduce the resulting capital gain for tax purposes.\n\nHistorical Adjustments: For assets like real estate, the trustee must determine the original purchase price and add the cost of any capital improvements or renovations made over the years. For property owned before 1972, the valuation as of December 31, 1971 (Valuation Day), serves as the starting point for the ACB.');

  addTaskContent('35', 'Investigate and list all outstanding debts, mortgages, and lines of credit',
    'Investigating and listing all outstanding debts is a critical fiduciary duty that ensures the net value of the estate is accurately determined before any distribution to beneficiaries occurs. This process requires the Estate Trustee to move beyond personal records and perform a formal verification of all liabilities.\n\nKey steps include:\n\n1. Formal Verification with Financial Institutions: The Estate Trustee must contact every bank or financial institution where the deceased had dealings to request a comprehensive list of all outstanding liabilities, including mortgages, revolving credit, and personal and business loans.\n\n2. Determining if Debts were Life-Insured: If a mortgage or loan had "creditor insurance," the lender may pay off the balance automatically upon receiving proof of death, effectively removing that liability from the estate\'s burden.\n\n3. Searching for "Hidden" Liabilities: Best practice involves requesting a report from Equifax or TransUnion to identify all credit facilities registered in the deceased\'s name. For business owners, investigate if the deceased provided personal guarantees for corporate debts.\n\n4. Legal Protection via "Advertisement for Creditors": To protect themselves from personal liability, Estate Trustees should publish a formal "Notice to Creditors" in local newspapers. This provides a legal window for any unknown creditors to come forward.');

  addTaskContent('36', 'Apply for and collect the CPP/QPP death benefit (up to $2,500)',
    'The estate trustee must complete the following administrative tasks:\n• Identify the Correct Form: For CPP, the trustee must use Form ISP-1200. For QPP, application forms are provided through the Retraite Québec (formerly Régie des rentes du Québec) website or the clicSÉQUR portal.\n• Gather Documentation: The application must be accompanied by official proof of death. This typically includes a death certificate issued by the province or funeral director, a burial certificate, or a copy of the coroner\'s report.\n• Submit to Authorities: Forms are submitted to Service Canada (for CPP) or Retraite Québec (for QPP).');

  addTaskContent('37', 'Contact the deceased\'s employer to inquire about pension plans and retiree benefits',
    'This document should provide the location of current and/or previous employment benefits to aid the Estate Trustee(s) in seeing what benefits and opportunities could be available to the client\'s beneficiaries or estate.');

  addTaskContent('38', 'File formal claims for all life insurance policies',
    'This document should provide the location of life insurance policies to aid the Estate Trustee(s) in seeing what benefits and opportunities could be available to the client\'s beneficiaries or estate.');

  addTaskContent('39', 'Search for missing life insurance policies through the OmbudService for Life & Health Insurance if necessary',
    'The OmbudService for Life & Health Insurance (OLHI) is a national, independent, non-profit organization that provides complaint resolution and information services for consumers of Canadian life and health insurance. One of its most critical practical functions for an Estate Trustee (Executor or Liquidator) is assisting in the location of insurance policies that the deceased may have owned but for which no physical documentation can be found.');

  addTaskContent('40', 'Check for insurance coverage through professional associations or alumni groups',
    'This document should provide the location of professional associations or alumni groups benefits to aid the Estate Trustee(s) in seeing what benefits and opportunities could be available to the client\'s beneficiaries or estate.');

  addStageHeading('Stage 5: Administering the Estate');

  addTaskContent('41', 'Review the suitability of investments held within the estate',
    'Reviewing the suitability of investments is a core fiduciary duty that requires the Estate Trustee to manage assets in the best interests of beneficiaries while fulfilling the specific instructions of the Will. If the Will is silent, trustees must rely on provincial legislation, such as the Trustee Act, which generally mandates the "prudent investor" standard.\n\nKey considerations include:\n• The "even-hand rule," which dictates that trustees must act impartially between income beneficiaries and capital beneficiaries.\n• A general obligation to diversify the investment portfolio to an extent appropriate for the trust\'s circumstances.\n• Consulting with qualified investment professionals to mitigate potential short-term losses.\nFailure to adhere to these standards can result in significant personal liability.');

  addTaskContent('42', 'Determine which assets must be liquidated to meet cash requirements for debts and taxes',
    'This task requires the Estate Trustee (Executor) to strategically identify and sell specific estate assets to generate the cash necessary to settle last expenses, outstanding debts, and the final income tax liability. This is a critical fiduciary duty because fiduciaries are generally required to deal with all debts before any assets can be transferred to beneficiaries. Without sufficient liquid resources, an executor may be forced into "unpopular choices," such as borrowing funds with non-deductible interest or selling real estate and business interests at a significant discount due to a sense of urgency.');

  addTaskContent('43', 'Invest any surplus cash in allowable, low-risk "prudent investor" vehicles until final distribution',
    'The task of investing surplus cash during estate administration is a core fiduciary duty intended to ensure that trust property remains productive while awaiting final distribution. Following the standard of the "prudent person" or "prudent investor," the Estate Trustee is expected to invest any funds not immediately required for debts or taxes in a manner that preserves the estate\'s value while seeking a reasonable return. Historically, many jurisdictions relied on rigid "legal lists" of authorized investments, but these have largely been replaced by the prudent investor standard, which requires the fiduciary to exercise the care, skill, diligence, and judgment that a person of ordinary prudence would use in managing the property of others.');

  addTaskContent('44', 'Assist in establishing and funding any testamentary trusts created in the Will',
    'A testamentary trust is a special legal relationship created through specific instructions in a Will that separates the legal ownership of assets from their beneficial use and enjoyment. The Estate Trustee (or executor) is responsible for ensuring the trust is validly established by meeting the "three certainties": clear intention to create the trust, an identifiable subject matter (property), and clear objects (beneficiaries). This process usually requires probating the Will to confirm the executor\'s legal authority to manage the deceased\'s assets and eventually deliver them to the appointed trustees. Once established, the trustee must manage the property according to the specific terms dictated in the Will, which should outline the amount of capital to be held, the duration of the trust, and the precise conditions for distributing income or capital to heirs.');

  addTaskContent('45', 'Cancel government benefits (CPP, QPP, OAS) to avoid overpayment',
    'This will be handled alongside task 36.');

  addTaskContent('46', 'Apply for survivor\'s pensions or children\'s benefits for eligible family members',
    'Applying for survivor\'s pensions and children\'s benefits is a time-sensitive duty for the representative to ensure the ongoing financial stability of the deceased\'s dependents. These benefits are not automatic and require formal applications supported by original proof-of-death documentation.\n\nKey benefits include:\n\n1. Apply for CPP & QPP Survivor Benefits: This will be done alongside tasks 36 and 45.\n\n2. Children\'s and Orphan Benefits: Dependent children of a deceased contributor may qualify for a monthly flat-rate benefit, often referred to as an orphan benefit. Eligibility is generally limited to children under age 18, or those between 18 and 25 who are attending a recognized school or university full-time. There is an exception for children who were disabled and dependent on the deceased before age 18; they may continue to receive the benefit as long as the dependency persists.\n\n3. Supplemental and Private Benefits: Low-income survivors aged 60 to 64 may qualify for the Allowance for the Survivor via the Old Age Security (OAS) program, which provides support until they turn 65. Additionally, most employer-sponsored Defined Benefit plans are mandated by law to provide a survivor benefit of at least 60% to a surviving spouse unless it was previously waived.');

  addTaskContent('47', 'Notify the CRA to discontinue or transfer GST/HST credits and child tax benefits',
    'The primary objective is to ensure that the government is informed of the death so that tax-related social benefits are handled correctly, preventing overpayments and ensuring continuity for survivors.\n\nThe key components include:\n• Cancellation of GST/HST Credits: The representative must contact the CRA and/or Revenue Quebec to cancel any future GST/HST credit payments scheduled for the deceased.\n• Notification of Child Tax Benefits: If the deceased was receiving federal or Quebec child tax benefits, the representative must advise the local CRA Tax Centre of the death. In Quebec, the CRA will forward this information to the Régie des rentes du Québec.\n• Transfer of Eligibility: A critical part of this task is determining if a surviving spouse or the children\'s new legal guardians are entitled to receive these benefits. This ensures that financial support for dependents continues under the correct recipient.');

  addTaskContent('48', 'Return the deceased\'s Social Insurance Card, Passport, Health Card and Driver\'s License',
    'This role is an administrative and security-related duty of the Estate Trustee (Executor or Liquidator) to officially cancel the deceased\'s government-issued identity documents and prevent their misuse.\n\nThe specific requirements include:\n• Documentation and Recording: Before returning the physical cards, the trustee should photocopy or record the numbers of the Social Insurance Card and Passport. These numbers are often required later to apply for survivor benefits, pensions, or for future reference with tax authorities.\n• Returning the Social Insurance Card: The physical card must be sent to the local Human Resources Centre of Canada along with a copy of the proof of death and a brief explanatory note.\n• Returning the Passport: The passport should be returned to a local or central passport office (Foreign Affairs and International Trade Canada) with a letter of explanation.\n• Broader Responsibility: This task often extends to other identification, such as the driver\'s licence (which may yield a refund for the unused portion of the term) and the provincial health card.\n\nPerforming this duty ensures that federal departments are formally notified of the death and helps protect the estate from potential identity theft or fraud.');

  addTaskContent('49', 'Pay all legitimate debts and settle creditor claims once the advertisement period expires',
    'The role of paying legitimate debts and settling creditor claims is a mandatory fiduciary duty that must be completed prior to the final distribution of assets to beneficiaries. This process typically begins once the formal "Notice to Creditors" advertisement period has expired, which ensures that all potential claimants have had a legal window to come forward.\n\nKey aspects include:\n• Priority of Payment: The Estate Trustee is legally required to satisfy all outstanding liabilities and taxes first, as debts take precedence over testamentary gifts or other distributions.\n• Protection from Personal Liability: Advertising for creditors is a critical safeguard.\n• Verification and Documentation: The representative must investigate the legitimacy of all claims and maintain detailed receipts for every payment made.');

  addTaskContent('50', 'Obtain and file official receipts for every debt payment made by the estate',
    'The role of obtaining and filing official receipts for every debt payment is a mandatory administrative and fiduciary duty performed by the Estate Trustee (or liquidator) during the estate settlement process. It requires the representative to secure formal documentation for every liability, debt, or legitimate claim settled before the final distribution of assets. This practice ensures the creation of a verifiable paper trail for every financial transaction made on behalf of the estate.\n\nThese receipts serve as essential evidence for the "passing of accounts," which is a formal review where the trustee must prove to the beneficiaries or the court that the estate was managed properly. Meticulous record-keeping and filing of receipts act as a primary legal safeguard for the personal representative.');

  addStageHeading('Stage 6: Tax Compliance');

  addTaskContent('51', 'Obtain a copy of the last income tax return filed by the deceased',
    'The previous return is necessary to ensure all prior-year tax obligations were met, as the legal representative is responsible for filing returns for any years the deceased may have missed. To obtain these records from the CRA or Revenue Quebec, the trustee must provide proof of death, the deceased\'s Social Insurance Number, and official estate documentation confirming their legal authority.\n\nThe representative can also utilize the CRA\'s "My Account" service to view previous returns, tax slips, and registered contribution room, which aids in the accurate preparation of the final (terminal) tax return. Completing this role promptly is critical, as failure to file outstanding returns and pay any associated balances can result in interest charges and late-filing penalties assessed against the estate.');

  addTaskContent('52', 'Complete and file the Final/Terminal Return (January 1 to date of death)',
    'The Final Return (also known as the terminal return) is a required T1 Individual Income Tax Return that covers the period from January 1 of the year of death up to and including the date of death. As the legal representative, the estate trustee or liquidator is responsible for filing this return and ensuring all associated taxes are paid to the Canada Revenue Agency. The return must report all income earned or received during this period, including salary, wages, pension income, and interest from investments like GICs and bonds.\n\nA primary feature of the final return is the deemed disposition of capital property, which treats the deceased as having sold all capital assets at fair market value (FMV) immediately before death. Filing deadlines for the final return are determined by the date of death: if death occurs between January 1 and October 31, the return is due by April 30 of the following year; if death occurs between November 1 and December 31, it is due six months after the date of death.');

  addTaskContent('53', 'Determine if an RRSP contribution should be made for the year of death',
    'Although contributions cannot be made to a deceased person\'s own RRSP after their passing, the estate representative can claim a deduction for contributions the individual made personally prior to death, provided they were under the age of 72. A primary tax-saving strategy is to use the deceased\'s unused RRSP contribution room to make a posthumous contribution to a surviving spouse\'s or common-law partner\'s RRSP. To be valid, this contribution must occur in the year of death or within the first 60 days of the following calendar year.\n\nThis strategy allows the representative to claim a deduction on the deceased individual\'s final (terminal) income tax return. This is particularly useful for minimizing the high tax burden that typically results from the deemed disposition of all capital property and the mandatory deregistration of registered plans.');

  addTaskContent('54', 'Evaluate and file optional returns, such as a "Rights or Things" return, to save taxes',
    'Filing optional returns is a tax-saving strategy for an estate trustee that involves reporting specific types of income on separate returns rather than the required terminal return. The most common is the "Rights or Things" return, which captures income the deceased was entitled to but had not yet received at the time of death, such as vacation pay, declared but unpaid dividends, matured bond coupons, or professional work-in-progress.\n\nThe primary financial benefit of this strategy is the ability to claim basic personal and other tax credits a second time, effectively multiplying the tax-free portion of the deceased\'s income. By splitting income across multiple returns, the representative can move funds that would otherwise be taxed at the individual\'s highest marginal rate into lower or nil tax brackets on the separate filing.');

  addTaskContent('55', 'File annual T3 Trust Income Tax Returns for the estate until it is fully wound up',
    'The requirement to file annual T3 Trust Income Tax and Information Returns arises because the sources designate an estate as a trust for tax purposes, meaning it is treated as a separate taxpayer. As the legal representative, the Estate Trustee is responsible for filing these returns to report all income and capital gains earned by the estate after the date of death. This return serves a dual function: it calculates the estate\'s tax liability and acts as an information return to report income allocated to beneficiaries through the issuance of T3 slips.\n\nThe estate must continue filing annually until it is fully wound up, which occurs upon the final distribution of all assets to the beneficiaries. Once the final return is processed and a Notice of Assessment is issued, the representative should apply for a Tax Clearance Certificate (Form TX19).');

  addTaskContent('56', 'Pay all required income taxes from estate funds',
    'The Estate Trustee (or liquidator) is legally responsible for ensuring that all outstanding income taxes of the deceased and the estate are paid directly from estate funds before distributing assets to beneficiaries. This includes settling the liability from the final (terminal) return, which typically captures income up to the date of death and the deemed disposition of capital property and registered plans at fair market value.\n\nIf the estate lacks sufficient liquid cash to cover these liabilities, the representative may be required to liquidate investments or other property to satisfy the debt to the CRA. Adhering to this priority is critical because a representative who distributes funds to heirs without first paying all taxes can be held personally liable for the unpaid amounts. Once all required returns are assessed and taxes are paid, the representative should obtain a Tax Clearance Certificate (Form TX19) as final confirmation that they are officially relieved of this personal liability.');

  addTaskContent('57', 'Apply for and obtain a Tax Clearance Certificate from the CRA (and Revenue Quebec)',
    'A Tax Clearance Certificate (Form TX19) is a written confirmation issued by the CRA (and Revenu Québec, if applicable) certifying that all tax liabilities of a deceased individual or their estate have been settled or that acceptable security for payment has been provided. These liabilities include not only income taxes for the year of death and all prior years but also CPP contributions, EI premiums, interest, and penalties. Obtaining this certificate is a critical fiduciary duty because it serves as the representative\'s only assurance that they are officially relieved of personal liability for any future unpaid taxes of the deceased or the estate.\n\nThe application process begins only after the Notice of Assessment for the final (terminal) return—and any required T3 Trust returns—has been received and all identified debts have been paid. If the representative distributes property without first obtaining this certificate, they become personally liable for any unpaid taxes, interest, and penalties to the extent of the value of the property distributed.');

  addStageHeading('Stage 7: Distribution and Closing');

  addTaskContent('58', 'Confirm sufficient funds remain to pay all debts and taxes before any distribution',
    'Confirming that sufficient funds remain to satisfy all obligations is a mandatory fiduciary duty that must be completed prior to any distribution of assets to beneficiaries. This process ensures the estate follows the legal priority of payment, where all outstanding liabilities and taxes must be settled before testamentary gifts can be fulfilled.\n\nKey aspects include:\n• Solvency and Liquidity: The representative must take a comprehensive inventory of all assets and liabilities to determine the estate\'s financial position. If the estate lacks enough liquid cash to pay debts and taxes, the executor is responsible for liquidating investments or property in a prudent manner to generate the necessary funds.\n• Risk of Personal Liability: Distributing assets prematurely is a significant legal risk. If an executor distributes property to heirs before all taxes and debts are paid, they become personally liable for the unpaid amounts to the extent of the value of the property distributed.\n• Holdbacks and Tax Clearance: It is standard practice to withhold a "holdback" amount—sufficient funds to cover any unexpected liabilities or final tax assessments—until a Tax Clearance Certificate is obtained from the CRA (and Revenue Quebec, if applicable). This certificate is the representative\'s official assurance that they are relieved of personal liability for the deceased\'s taxes.\n• Abatement during Shortfalls: If the executor confirms there are insufficient funds to satisfy both debts and all gifts in the Will, the gifts must be reduced in a specific legal order known as abatement.');

  addTaskContent('59', 'Distribute specific bequests of personal and household effects',
    'A specific bequest is a gift under a Will involving a particular item of personal property or a specific amount of cash. These bequests typically encompass personal and household effects such as furniture, jewelry, and collectibles. Rather than itemizing every heirloom within the Will itself, testators often utilize a memorandum clause that refers to a separate list or letter specifying their wishes for personal possessions.\n\nWhen distributing these items, the representative is required to obtain signed official receipts from each beneficiary to support the final accounting of the estate.');

  addTaskContent('60', 'Obtain signed receipts for every item distributed',
    'Obtaining signed receipts for every item distributed is a mandatory fiduciary duty of an executor or liquidator to ensure a verifiable paper trail for all asset transfers. This requirement applies to specific bequests of personal and household belongings, as well as the final distribution of the remaining residue of the estate.\n\nThese receipts are essential for:\n• Final Accounting: They serve as critical evidence for the final report to beneficiaries.\n• Passing of Accounts: If the representative is required to undergo a formal "passing of accounts" before a court, these receipts provide the necessary documentation.\n• Protection from Personal Liability: Maintaining these records acts as a primary legal safeguard.\n• Distributions for Minors: When dealing with minor beneficiaries, trustees must substantiate expenditures made for the child\'s benefit by keeping original receipts issued by third parties or guardians.\n\nOnce the final distribution is complete and receipts are obtained, it is standard practice to also secure signed releases from residual beneficiaries, confirming their satisfaction and waiving their right to sue the representative in the future.');

  addTaskContent('61', 'Calculate and prepare a final reckoning of executor compensation',
    'The role of an executor involves calculating and preparing a formal reckoning of their compensation, a task that should be completed before the final distribution of the estate. This process is governed by the Will, provincial legislation, and agreements with beneficiaries.\n\nExecutor fees are typically determined by one of three sources:\n• The Will: The testator may include a "charging provision" that explicitly sets the remuneration rate.\n• Beneficiary Agreement: Adult and capable beneficiaries may unanimously agree on a fee.\n• Statutory Guidelines: In the absence of specific instructions, provincial laws provide that the fee must be "fair and reasonable" in proportion to the estate\'s size and the work performed. Fees generally range from 3% to 5% of the total value of the estate.');

  addTaskContent('62', 'Distribute the residue of the estate to beneficiaries as specified in the Will',
    'The residue of an estate is the portion that remains after all funeral expenses, debts, administrative fees, and taxes have been paid, and all specific bequests or devises have been fulfilled. The Estate Trustee is responsible for ensuring the remaining assets are divided among the residual beneficiaries exactly as specified in the Will.\n\nKey considerations include:\n• Priority of Payment: The trustee has a mandatory fiduciary duty to satisfy all creditor claims and tax liabilities first.\n• Tax Clearance: Before the final distribution, the trustee should obtain a Tax Clearance Certificate.\n• Final Accounting and Releases: Residual beneficiaries have the legal right to receive and approve a full accounting of the executor\'s handling of the estate.');

  addTaskContent('63', 'Prepare a Final Accounting detailing all assets, liabilities, income, and expenses',
    'The final accounting is the executor\'s mandatory terminal responsibility to provide beneficiaries with a comprehensive report of their management of the estate. This accounting serves as the basis for beneficiaries to approve the administration and release the executor from further liability.\n\nCore components include:\n• Asset Inventory: A list of all assets owned by the deceased at the date of death compared to those remaining at the end of the accounting period.\n• Financial Activity: Detailed statements of all assets purchased and sold, as well as all income generated.\n• Liabilities and Expenses: A record of all debts paid, funeral expenses, and ongoing administrative costs.\n• Executor Compensation: A formal reckoning of compensation.\n• Distributions: A clear summary of how assets were distributed to beneficiaries.');

  addTaskContent('64', 'Present the accounting to adult beneficiaries for approval',
    'Presenting the accounting to adult beneficiaries is a critical closing step in estate administration where the representative provides a comprehensive report of all assets, liabilities, and distributions for review.\n\nKey elements include:\n• Written Approval and Releases: The executor seeks written approval of the accounts and signed release forms from residual beneficiaries.\n• Verification of Management: Beneficiaries use this review to ensure debts and taxes have been paid and that they are receiving their correct share.\n• Approval of Compensation: Beneficiaries are entitled to review and approve the executor\'s level of compensation.\n• Judicial Recourse: If an adult beneficiary refuses to approve the accounts, the executor may be forced to apply for a formal "passing of accounts" in court.');

  addTaskContent('65', 'Have adult beneficiaries sign Release Forms to waive their right to sue the executor',
    'Obtaining signed release forms from adult beneficiaries is a standard and essential practice used to conclude the administration of an estate. These documents confirm that the beneficiaries have reviewed and approved the final accounting, which details all assets, liabilities, and expenses handled by the executor. By signing a release, a beneficiary acknowledges they have received their inheritance in full satisfaction of their entitlement and waives their right to sue the executor in the future regarding the estate\'s administration.\n\nThis process serves as a primary legal safeguard, protecting the personal representative from future claims of financial mismanagement or breach of fiduciary duty. If an adult beneficiary refuses to sign a release, or if a beneficiary is a minor who lacks the legal capacity to provide a valid discharge, the executor may be forced to apply for a formal "passing of accounts" in court. This judicial review allows the court to approve the executor\'s actions and compensation, providing a court-ordered protection from subsequent litigation that a signed release would otherwise provide.');

  addTaskContent('66', '(If necessary) Apply for a court order for the "passing of accounts" if a beneficiary refuses to sign a release',
    'A "passing of accounts" is a formal court application where a judge reviews and judicially approves the representative\'s management of the estate, including all receipts, disbursements, and claimed compensation. This step becomes necessary as a legal safeguard if a residual beneficiary refuses to sign a release or lacks the capacity to do so (such as a minor), as it provides the representative with a court-ordered discharge from future liability.\n\nWhile a court order allows the representative to proceed with final distribution without a specific beneficiary\'s signature, it is typically viewed as a last resort because the process often causes significant delays and incurs legal costs that reduce the overall value of the estate.');
